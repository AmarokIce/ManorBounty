...
import com.djinfinite.manors_bounty.block.CherriesPieBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class CherriesPieTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(CherriesPieBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(CherriesPieBlock.BLOCKSTATE);

	public CherriesPieTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHERRIES_PIE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(CherriesPieBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(CherriesPieBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("cherries_pie");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Big Cherries Pie");
	}
