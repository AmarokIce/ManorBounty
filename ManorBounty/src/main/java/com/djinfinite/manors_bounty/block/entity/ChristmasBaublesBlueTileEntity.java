...
import com.djinfinite.manors_bounty.block.ChristmasBaublesBlueBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasBaublesBlueTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ChristmasBaublesBlueBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ChristmasBaublesBlueBlock.BLOCKSTATE);

	public ChristmasBaublesBlueTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_BAUBLES_BLUE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ChristmasBaublesBlueBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasBaublesBlueBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_baubles_blue");
	}
