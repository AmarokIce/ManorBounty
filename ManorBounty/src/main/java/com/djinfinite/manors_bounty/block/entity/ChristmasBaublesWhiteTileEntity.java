...
import com.djinfinite.manors_bounty.block.ChristmasBaublesWhiteBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasBaublesWhiteTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ChristmasBaublesWhiteBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ChristmasBaublesWhiteBlock.BLOCKSTATE);

	public ChristmasBaublesWhiteTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_BAUBLES_WHITE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ChristmasBaublesWhiteBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasBaublesWhiteBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_baubles_white");
	}
