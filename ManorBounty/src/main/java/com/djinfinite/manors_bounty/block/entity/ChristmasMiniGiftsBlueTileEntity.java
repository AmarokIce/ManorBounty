...
import com.djinfinite.manors_bounty.block.ChristmasMiniGiftsBlueBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasMiniGiftsBlueTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ChristmasMiniGiftsBlueBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ChristmasMiniGiftsBlueBlock.BLOCKSTATE);

	public ChristmasMiniGiftsBlueTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_MINI_GIFTS_BLUE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ChristmasMiniGiftsBlueBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasMiniGiftsBlueBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_mini_gifts_blue");
	}
