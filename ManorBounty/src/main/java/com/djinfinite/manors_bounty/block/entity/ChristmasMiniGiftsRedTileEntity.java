...
import com.djinfinite.manors_bounty.block.ChristmasMiniGiftsRedBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasMiniGiftsRedTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ChristmasMiniGiftsRedBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ChristmasMiniGiftsRedBlock.BLOCKSTATE);

	public ChristmasMiniGiftsRedTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_MINI_GIFTS_RED.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ChristmasMiniGiftsRedBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasMiniGiftsRedBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_mini_gifts_red");
	}
