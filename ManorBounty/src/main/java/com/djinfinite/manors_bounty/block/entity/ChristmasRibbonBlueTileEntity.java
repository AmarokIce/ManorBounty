...
import com.djinfinite.manors_bounty.block.ChristmasRibbonBlueBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasRibbonBlueTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ChristmasRibbonBlueTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_RIBBON_BLUE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasRibbonBlueBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_ribbon_blue");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Christmas Ribbon Blue");
	}
