...
import com.djinfinite.manors_bounty.block.ChristmasRibbonRedBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasRibbonRedTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ChristmasRibbonRedTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_RIBBON_RED.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasRibbonRedBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_ribbon_red");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Christmas Ribbon Red");
	}
