...
import com.djinfinite.manors_bounty.block.ChristmasRibbonWhiteBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasRibbonWhiteTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ChristmasRibbonWhiteTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_RIBBON_WHITE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasRibbonWhiteBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_ribbon_white");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Christmas Ribbon White");
	}
