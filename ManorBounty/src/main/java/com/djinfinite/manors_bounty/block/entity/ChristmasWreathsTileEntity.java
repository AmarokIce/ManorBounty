...
import com.djinfinite.manors_bounty.block.ChristmasWreathsBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ChristmasWreathsTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public ChristmasWreathsTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CHRISTMAS_WREATHS.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(ChristmasWreathsBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("christmas_wreaths");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Christmas Wreaths");
	}
