...
import com.djinfinite.manors_bounty.block.CreamWithMushroomSoupBlockBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class CreamWithMushroomSoupBlockTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public CreamWithMushroomSoupBlockTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.CREAM_WITH_MUSHROOM_SOUP_BLOCK.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(CreamWithMushroomSoupBlockBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("cream_with_mushroom_soup_block");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Cream with Mushroom Soup");
	}
