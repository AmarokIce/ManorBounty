...
public class DragonFruitSliceBlockTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(DragonFruitSliceBlockBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(DragonFruitSliceBlockBlock.BLOCKSTATE);

	public DragonFruitSliceBlockTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.DRAGON_FRUIT_SLICE_BLOCK.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(DragonFruitSliceBlockBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(DragonFruitSliceBlockBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("dragon_fruit_slice_block");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Dragon Fruit Slice");
	}
