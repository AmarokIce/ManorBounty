...
import com.djinfinite.manors_bounty.block.GiftShortGreenBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class GiftShortGreenTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public GiftShortGreenTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.GIFT_SHORT_GREEN.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(GiftShortGreenBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("gift_short_green");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Gift Short Green");
	}
