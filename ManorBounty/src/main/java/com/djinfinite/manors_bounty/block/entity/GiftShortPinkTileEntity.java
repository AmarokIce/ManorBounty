...
import com.djinfinite.manors_bounty.block.GiftShortPinkBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class GiftShortPinkTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public GiftShortPinkTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.GIFT_SHORT_PINK.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(GiftShortPinkBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("gift_short_pink");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Gift Short Pink");
	}
