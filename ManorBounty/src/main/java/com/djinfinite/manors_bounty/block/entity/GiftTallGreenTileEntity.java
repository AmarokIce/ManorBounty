...
import com.djinfinite.manors_bounty.block.GiftTallGreenBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class GiftTallGreenTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public GiftTallGreenTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.GIFT_TALL_GREEN.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(GiftTallGreenBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("gift_tall_green");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Gift Tall Green");
	}
