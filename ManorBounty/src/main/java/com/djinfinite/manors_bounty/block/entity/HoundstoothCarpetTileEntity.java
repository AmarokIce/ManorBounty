...
import com.djinfinite.manors_bounty.block.HoundstoothCarpetBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class HoundstoothCarpetTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(HoundstoothCarpetBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(HoundstoothCarpetBlock.BLOCKSTATE);

	public HoundstoothCarpetTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.HOUNDSTOOTH_CARPET.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(HoundstoothCarpetBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(HoundstoothCarpetBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("houndstooth_carpet");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Houndstooth Carpet");
	}
