...
import com.djinfinite.manors_bounty.block.MagicCircleCarpetBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class MagicCircleCarpetTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(MagicCircleCarpetBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(MagicCircleCarpetBlock.BLOCKSTATE);

	public MagicCircleCarpetTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.MAGIC_CIRCLE_CARPET.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(MagicCircleCarpetBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(MagicCircleCarpetBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("magic_circle_carpet");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Magic Circle Carpet");
	}
