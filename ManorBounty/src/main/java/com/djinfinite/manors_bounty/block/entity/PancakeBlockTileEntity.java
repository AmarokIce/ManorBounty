...
import com.djinfinite.manors_bounty.block.PancakeBlockBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class PancakeBlockTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(PancakeBlockBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(PancakeBlockBlock.BLOCKSTATE);

	public PancakeBlockTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.PANCAKE_BLOCK.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(PancakeBlockBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(PancakeBlockBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("pancake_block");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Pancake");
	}
