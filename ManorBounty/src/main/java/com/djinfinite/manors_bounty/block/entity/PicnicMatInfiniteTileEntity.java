...
import com.djinfinite.manors_bounty.block.PicnicMatInfiniteBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class PicnicMatInfiniteTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public PicnicMatInfiniteTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.PICNIC_MAT_INFINITE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		String animationprocedure = ("" + this.getBlockState().getValue(PicnicMatInfiniteBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("picnic_mat_infinite");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Picnic Mat Infinite");
	}
