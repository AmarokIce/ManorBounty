...
import com.djinfinite.manors_bounty.block.PicnicMatBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class PicnicMatTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(PicnicMatBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(PicnicMatBlock.BLOCKSTATE);

	public PicnicMatTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.PICNIC_MAT.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(PicnicMatBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(PicnicMatBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("picnic_mat");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Picnic Mat");
	}
