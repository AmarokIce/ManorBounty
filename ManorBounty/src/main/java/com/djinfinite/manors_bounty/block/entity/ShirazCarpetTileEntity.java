...
import com.djinfinite.manors_bounty.block.ShirazCarpetBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class ShirazCarpetTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(ShirazCarpetBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(ShirazCarpetBlock.BLOCKSTATE);

	public ShirazCarpetTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.SHIRAZ_CARPET.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(ShirazCarpetBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(ShirazCarpetBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("shiraz_carpet");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Shiraz Carpet");
	}
