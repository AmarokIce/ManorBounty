...
import com.djinfinite.manors_bounty.block.TrufflePieBlock;

import javax.annotation.Nullable;

import java.util.stream.IntStream;

public class TrufflePieTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());
	public int blockstateNew = this.getBlockState().getValue(TrufflePieBlock.BLOCKSTATE);
	private int blockstateOld = this.getBlockState().getValue(TrufflePieBlock.BLOCKSTATE);

	public TrufflePieTileEntity(BlockPos pos, BlockState state) {
		super(ModBlockEntities.TRUFFLE_PIE.get(), pos, state);
	}

	private PlayState predicate(AnimationState event) {
		blockstateNew = this.getBlockState().getValue(TrufflePieBlock.BLOCKSTATE);
		if (blockstateOld != blockstateNew) {
			event.getController().forceAnimationReset();
			blockstateOld = blockstateNew;
			return PlayState.STOP;
		}
		String animationprocedure = ("" + this.getBlockState().getValue(TrufflePieBlock.ANIMATION));
	...
	public @NotNull Component getDefaultName() {
		return Component.literal("truffle_pie");
	}
	...
	public @NotNull Component getDisplayName() {
		return Component.literal("Big Truffle Pie");
	}
