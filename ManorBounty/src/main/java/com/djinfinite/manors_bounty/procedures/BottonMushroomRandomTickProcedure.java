...
public class BottonMushroomRandomTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		double chance;
		chance = Mth.nextInt(RandomSource.create(), 1, 100);
		if (chance <= 8) {
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == ModBlocks.BOTTON_MUSHROOM.get()) {
				world.setBlock(BlockPos.containing(x, y, z), ModBlocks.BOTTON_MUSHROOM_2.get().defaultBlockState(), 3);
			} else if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == ModBlocks.BOTTON_MUSHROOM_2.get()) {
