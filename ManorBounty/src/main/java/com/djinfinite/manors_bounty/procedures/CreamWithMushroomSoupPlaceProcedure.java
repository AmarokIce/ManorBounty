...
public class CreamWithMushroomSoupPlaceProcedure {
	public static InteractionResult execute(LevelAccessor world, double x, double y, double z, Direction direction, Entity entity, ItemStack itemstack) {
		...
		if (world.getBlockState(BlockPos.containing(x, y, z)).isFaceSturdy(world, BlockPos.containing(x, y, z), Direction.UP) && (new Object() {
			...
			world.setBlock(BlockPos.containing(x, y + 1, z), ModBlocks.CREAM_WITH_MUSHROOM_SOUP_BLOCK.get().defaultBlockState(), 3);
			...
			world.setBlock(BlockPos.containing(x, y, z), ModBlocks.CREAM_WITH_MUSHROOM_SOUP_BLOCK.get().defaultBlockState(), 3);
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
