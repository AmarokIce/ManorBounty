...
public class DragonFruitBlockPlaceProcedure {
	public static InteractionResult execute(LevelAccessor world, double x, double y, double z, Direction direction, Entity entity, ItemStack itemstack) {
		...
		if (world.getBlockState(BlockPos.containing(x, y, z)).isFaceSturdy(world, BlockPos.containing(x, y, z), Direction.UP) && (new Object() {
			...
			world.setBlock(BlockPos.containing(x, y + 1, z), ModBlocks.DRAGON_FRUIT_BLOCK.get().defaultBlockState(), 3);
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y + 1, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.bamboo.place")), SoundSource.BLOCKS, 1, 1);
				} else {
					_level.playLocalSound(x, (y + 1), z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.bamboo.place")), SoundSource.BLOCKS, 1, 1, false);
				}
			}
			...
			world.setBlock(BlockPos.containing(x, y, z), ModBlocks.DRAGON_FRUIT_BLOCK.get().defaultBlockState(), 3);
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y + 1, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.bamboo.place")), SoundSource.BLOCKS, 1, 1);
				} else {
