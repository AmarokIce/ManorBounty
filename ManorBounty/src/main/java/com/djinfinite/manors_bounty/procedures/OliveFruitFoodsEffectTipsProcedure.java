...
public class OliveFruitFoodsEffectTipsProcedure {
	@OnlyIn(Dist.CLIENT)
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, Entity entity, ItemStack itemstack, List<Component> tooltip) {
		...
		if (DoFruitEffectEnableCheckProcedure.execute()) {
			if (itemstack.is(ItemTags.create(new ResourceLocation("manors_bounty:olive_fruit_foods")))) {
                Cooldown = (Component.translatable("tips.manors_bounty.fruit_effect_cooldown_time_get").getString()).replace(" %1 ",
                        new java.text.DecimalFormat("##").format((entity.getCapability(ManorsBountyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ManorsBountyModVariables.PlayerVariables())).MB_OliveFruitCooldown / 20));
                FoodsCooldown = (Component.translatable("tips.manors_bounty.fruit_effect_cooldown_time_get").getString()).replace(" %1 ",
                        new java.text.DecimalFormat("##").format((entity.getCapability(ManorsBountyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ManorsBountyModVariables.PlayerVariables())).MB_OliveFruitFoodsCooldown / 20));
                CooldownCheck = (entity.getCapability(ManorsBountyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ManorsBountyModVariables.PlayerVariables())).MB_OliveFruitCooldown > 0;
                FoodsCooldownCheck = (entity.getCapability(ManorsBountyModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ManorsBountyModVariables.PlayerVariables())).MB_OliveFruitFoodsCooldown != 0;
                if (itemstack.getItem() == ModItems.OLIVE_FRUIT.get()) {
					if (Screen.hasShiftDown()) {
						tooltip.add(Component.literal(((((Component.translatable("tips.manors_bounty.fruit_effect_raw_give").getString()).replace(" %1 ", Component.translatable("effect.manors_bounty.sea_touch").getString())).replace(" %2 ", "100"))
								.replace(" %3 ", "1"))));
				...
				if (itemstack.getItem() == ModItems.MARTINI.get()) {
					if (Screen.hasShiftDown()) {
						tooltip.add(Component.literal(
								(((Component.translatable("tips.manors_bounty.olive_effect_time_add").getString()).replace(" %1 ", new java.text.DecimalFormat("##").format(FruitsEffectConfig.FOODS_VALUE_X.get() * 100)))
										.replace(" %2 ", "I"))));
						tooltip.add(Component
								.literal(((((Component.translatable("tips.manors_bounty.fruit_effect_b_type_give_line_1").getString()).replace(" %1 ", "70")).replace(" %2 ", Component.translatable("effect.manors_bounty.sea_touch").getString()))
						...
						tooltip.add(Component.literal(((((Component.translatable("tips.manors_bounty.fruit_effect_b_type_give_line_3").getString()).replace(" %1 ", "30")).replace(" %2 ", "1")).replace(" %3 ",
								Component.translatable("effect.manors_bounty.sea_touch").getString()))));
