...
class IceCreamMachineMenu(pContainerId: Int, val tile: TileIceCreamMachine, val playerInv: Inventory) : AbstractContainerMenu(ModMenus.ICE_CREAM, pContainerId) {
    constructor(id: Int, inv: Inventory, buf: FriendlyByteBuf): this(id, inv.player.level().getBlockEntity(buf.readBlockPos()) as TileIceCreamMachine, inv)

    init {
        this.addSlot(SlotItemHandler(this.tile.inventory, 0, 38, 118))
        this.addSlot(SlotItemHandler(this.tile.inventory, 1, 104, 118))
        this.addSlot(SlotItemHandler(this.tile.inventory, 2, 143, 118))
        this.addSlot(SlotItemHandler(this.tile.inventory, 3, 141, 41))
